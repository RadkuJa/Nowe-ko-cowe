...
test('New Job', async (t) => {
  ...
  await t
    ...
    .typeText('textarea[name="description"]', 'Write some code in Angular')
    .typeText('input[name="company"]', 'Angularers')
    .typeText('input[name="email"]', 'philip@angularers.cz')
    .click(submitButton)
  // check title, table rows, and new job exists
  await t
    .expect(title.innerText).eql('All Jobs')
    .expect(tableRows.count).eql(4)
    .expect(Selector('tbody > tr').withText('Angular Developer').exists).ok();
});
